...
<script>
    ...
    export default {
        ...
        beforeCreate() {
            ...
            this.axiosResponse.forEach((item,index)=>{
                let valueType = item[index].value
                request.push({
                    name:valueType
                })
                console.log('value request:',request)
                axios.post(baseUrl + '/api/device/value/read').then(
                    (response)=>{
